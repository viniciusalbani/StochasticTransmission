...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% load('data_NYC_20220103.mat')
% load('data_NYC_20220328Boot.mat')
% load('data_NYC_20221011Boot.mat')
ReinfectionParam = 1./[30,60,180,360]; %days^-1
for lll = length(ReinfectionParam)
load(['dataReinf_NYC_20221015Boot_',num2str(lll),'.mat'])
...
extend = 1095;%60;
dates = [datetime(2020,04,05),datetime(2020,10,18),datetime(2021,07,10),datetime(2021,10,10),datetime(2021,11,05)];
...
len = 5; s = 0;
Death2 = DeathOld;
Death2 = [Death2(1:len0-s),ones(1,extend+s)*mean(Death2(len0-len-s:len0-s))];
params.factorDeath = @(t)interp1(t_extend,Death2,t);
...
len=45;
dt =1/365;
ObjFun = @(coef)ObjFunHeston_20220921(BETABoot(zz,len0-len-s:len0-s)',coef,t_actual(len0-len-s:len0-s),dt,BETABoot(zz,len0-len-s));
...
% len = 30; s = 0;
len=90;
ObjFun = @(coef)ObjFunCIRJumps_20220921(BETABoot(zz,len0-len-s:len0-s)',coef,t_actual(len0-len-s:len0-s),dt,BETABoot(zz,len0-len-s));
...
% len = 30; s = 0;
len=45;
ObjFun = @(coef)ObjFunLinearMR_20220921(BETABoot(zz,len0-len-s:len0-s)',coef,t_actual(len0-len-s:len0-s),dt,BETABoot(zz,len0-len-s));
coef0 = [0.1,BETABoot(zz,len0-len-s),0.01];
LB = [0,0,0.1];
UB = [10,10,10];
...
% len = 30; s = 0;
len=90;
ObjFun = @(coef)ObjFunMRLJumps_20220921(BETABoot(zz,len0-len-s:len0-s)',coef,t_actual(len0-len-s:len0-s),dt,BETABoot(zz,len0-len-s));
coef0 = [0.1,BETA(len0-s),0.5,0.5,0,1];
LB=[0,0,0.1,0,-10,0];
UB = [100,10,10,10,10,10];
...
yb2 = zeros(len1+1,length(yinit2));
yb2(1,:) = yinit2;
...
yinit2(1:NSamplesB) = ybOld(jj+1,1)*ones(1,NSamplesB);
yinit2(NSamplesB+1:2*NSamplesB) = ybOld(jj+1,2)*ones(1,NSamplesB);
yinit2(2*NSamplesB+1:3*NSamplesB) = ybOld(jj+1,3)*ones(1,NSamplesB);
yinit2(3*NSamplesB+1:4*NSamplesB) = ybOld(jj+1,4)*ones(1,NSamplesB);
yinit2(4*NSamplesB+1:5*NSamplesB) = ybOld(jj+1,5)*ones(1,NSamplesB);

yb2(jj+1,1:NSamplesB) = ybOld(jj+1,1)*ones(1,NSamplesB);
yb2(jj+1,NSamplesB+1:2*NSamplesB) = ybOld(jj+1,2)*ones(1,NSamplesB);
yb2(jj+1,2*NSamplesB+1:3*NSamplesB) = ybOld(jj+1,3)*ones(1,NSamplesB);
yb2(jj+1,3*NSamplesB+1:4*NSamplesB) = ybOld(jj+1,4)*ones(1,NSamplesB);
yb2(jj+1,4*NSamplesB+1:5*NSamplesB) = ybOld(jj+1,5)*ones(1,NSamplesB);
end
end 
NewInfections = sigma*yb2(:,NSamplesB+1:2*NSamplesB)*N;
...
for jj = 2:len1+1-len0-s+1
    ...
    if Atemp >= E2(ii)
        ...
        beta0 = BETA_MRJ(len0-s+jj-2,ii) + kappa*(theta-BETA_MRJ(len0-s+jj-2,ii))*ttemp + xi*sqrt(BETA_MRJ(len0-s+jj-2,ii))*sqrt(ttemp)*NN(jj-1,ii);
        ...
        beta0 = BETA_MRJ(len0-s+jj-2,ii) + kappa*(theta-BETA_MRJ(len0-s+jj-2,ii))*(jj*dt-ss(ii)) + xi*sqrt(BETA_MRJ(len0-s+jj-2,ii))*sqrt(jj*dt-ss(ii))*NN(jj-1,ii);
        A(jj,ii) = Atemp;
        ss(ii) = jj*dt;
    end
...
% Plotting_20221016
save(['dataReinf_NYC_20221015Boot_',num2str(lll),'FutWaves.mat'])
end